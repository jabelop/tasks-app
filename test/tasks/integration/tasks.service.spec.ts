...
describe('TasksService', () => {
  ...
  it('should save an task', async () => {
    expect((await service.create(task, 1000)).id).toBe(task.id);
  });

  it('should get all the tasks for the exisiting "user" user', async () => {
    expect(
      (await service.findAllForUserId('06c2f397-def1-4ef6-a0a9-476b482b82eb'))
        .length,
    ).toBeGreaterThan(0);
  });
  ...
  it('should update an task by id', async () => {
    ...
    expect((await service.update(task))?.title).toBe('Test3Updated');
  });
